...
lv_display_t *bsp_lvgl_init(void) {
    ...
    // ⚠ esp_lvgl_port 默认的 LVGL 任务栈只有 7168 字节，装不下 JPEG 解码。
    // lv_tjpgd 的 decoder_info() 在栈上开了 4096 字节工作缓冲（编译器为整个函数
    // 预留，即使走的是内存源分支），叠加渲染路径 lv_timer_handler → ... → 解码
    // 的栈帧就会越界，表现为一进带封面的页面就 Stack protection fault 整机重启。
    lvgl_port_cfg_t pc = ESP_LVGL_PORT_INIT_CONFIG();
    pc.task_stack = 16 * 1024;
